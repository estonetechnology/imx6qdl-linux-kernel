				     ...
				     struct snd_pcm_hw_params *params)
{
	...
	//snd_soc_dai_set_pll(codec_dai, 0, 0, pll_out, 24576000);
	snd_soc_dai_set_pll(codec_dai, 0, 0, pll_out, params_rate(params) * 256);
	
 	//ret = snd_soc_dai_set_sysclk(codec_dai, 1, 24576000, SND_SOC_CLOCK_IN);
	ret = snd_soc_dai_set_sysclk(codec_dai, 1, params_rate(params) * 256, SND_SOC_CLOCK_IN);

	if (ret < 0) {
		printk("imx6_hw_params_rt5640:failed to set the sysclk for codec side\n"); 
		return ret;
	}
				...
				struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_pll(codec_dai, 0, RT5640_PLL1_S_BCLK1,
				  params_rate(params) * 64 /*24000000*/,
				  params_rate(params) * 256);
	if (ret < 0) {
	...
	//snd_soc_dai_set_clkdiv(codec_dai, ROCKCHIP_DIV_BCLK, (params_rate(params) * 256 /4)/params_rate(params)-1);
	//snd_soc_dai_set_clkdiv(codec_dai, ROCKCHIP_DIV_MCLK, 3);
				     ...
				     struct snd_pcm_hw_params *params)
{
	...
	DBG("Enter:%s, %d, clk_frequency=%d, pll_out=%d\n", __FUNCTION__, __LINE__, data->clk_frequency, pll_out);
...
static int imx_hifi_hw_free(struct snd_pcm_substream *substream)
{
	...
	if (!priv->first_stream) {
		...
		ret = snd_soc_dai_set_pll(codec_dai, 0,/*WM8962_FLL*
				/*WM8962_FLL_MCLK*/0, 0, 0);
		if (ret < 0) {
...
#ifdef USE_RT5631
	data->dai.codec_dai_name = "rt5640-aif1";
#else
	data->dai.codec_dai_name = "rt5640-aif1";
#endif
