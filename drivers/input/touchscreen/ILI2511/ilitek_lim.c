...
char REVERT_X = 0;
char REVERT_Y = 0;
// Serge 09-03-17 mis DBG_FLAG à 1 pour tests
char DBG_FLAG = 1,DBG_COR; 
//#define ROTATE_FLAG
...
static int ilitek_report_data_3XX(void) {
	...
	int ret = 0, i = 0, j = 0, x = 0, y = 0, tmp = 0, tp_status = 0, packet = 0, max_point = 1;
...
static int ilitek_i2c_early_suspend(struct tp_device *tp_d)
{
...
/*
   ...
   nothing
 */
static int ilitek_i2c_late_resume(struct tp_device *tp_d)
{
	printk("%s\n", __func__);
...
static int ilitek_request_io_port(struct i2c_client *client)
{
	...
	#ifdef CONFIG_OF
	i2c.reset_gpio = of_get_named_gpio(np, "reset-gpio", 0);
	#else
	...
	#ifdef CONFIG_OF
	i2c.irq_gpio = of_get_named_gpio(np, "touch-gpio", 0);
	#else
