...
uint bcm_wlan_get_oob_irq(void)
{
	...
	if (gpio_request(g_WL_HOST_WAKE, "bcmdhd") < 0) {
		printf("%s: gpio_request failed\n", __FUNCTION__);
	}
	host_oob_irq = gpio_to_irq(g_WL_HOST_WAKE);
	if (gpio_direction_input(g_WL_HOST_WAKE) < 0 ) {
